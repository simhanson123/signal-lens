...
export async function executeSlashCommand(
  ...
  switch (req.command) {
    ...
    case "explain": {
      ...
      if (history.length === 0) {
        return { command: "explain", message: "No prior review found. Run `/review-mcp review` first." };
      }
    ...
    case "false-positive": {
      ...
      if (!findingId) {
        return { command: "false-positive", message: "Usage: `/review-mcp false-positive <finding-id> [reason]`" };
      }
    ...
    case "fix": {
      const findingId = req.args.split(/\s+/)[0];
      if (!findingId) {
        return { command: "fix", message: "Usage: `/review-mcp fix <finding-id>`" };
      }
